...
export default function AdminCategoriesPage() {
  ...
  async function toggleActive(cat: ICategory) {
    ...
    setCategories((prev) =>
      prev.map((c) => (c._id === cat._id ? { ...c, isActive: next } : c))
    );
      ...
      <div className="bg-white rounded-2xl border border-base-200 shadow-sm overflow-x-auto">
        {loading ? (
          ...
          <div className="text-center py-16 text-muted">
            No categories yet.{" "}
            <Link href="/admin/categories/new" className="link link-primary">Add one</Link>
          </div>
        ) : (
              ...
              {categories.map((cat) => {
                ...
                return (
                  <tr key={cat._id} className={active ? "" : "opacity-50"}>
                    ...
                    <td className="text-sm text-muted max-w-xs truncate">{cat.description || "—"}</td>
                    <td>
                      <input
                        type="checkbox"
                    ...
                    <td>
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/categories/${cat._id}`} className="btn btn-ghost btn-sm btn-square" title="Edit">
        ...
        <div className="modal modal-open">
          <div className="modal-box">
            ...
            <p className="py-4 text-muted">
              ...
              All cover images will be removed from Cloudinary.
            </p>
